...
const results = [
  ...
  result('tem_draw_anchor_lean_homeaway2_draw5_5_cap25', '平局锚点省注', 74.1, { roi: 75.9, hitRate: 30.4, coverage: 100, shapeHealth: 98, explainability: 78 }),
  ...
  result('tem_poisson_drawguard_context_v3_n2_draw7_5_cap35_p0_006', '赛前泊松EV平局保护', 77.7, { roi: 86.1, hitRate: 25, coverage: 100, shapeHealth: 94, explainability: 84 }),
  ...
  result('tem_source_consensus_poisson_context_v1_s2_c3_n3_cap6', '来源低赔泊松 3 格', 71.1, { roi: 62.9, hitRate: 35.7, coverage: 100, shapeHealth: 90, explainability: 90 }),
...
test('buildKnockoutStrategyEvolutionData appends one automatic experiment per flagship family', () => {
  ...
  assert.ok(data.families.every((family) => family.versions.length === 5));
  assert.deepEqual(data.families.map((family) => family.versions.at(-2).version), ['v3', 'v3', 'v3']);
  assert.deepEqual(data.families.map((family) => family.versions.at(-2).status), ['active', 'active', 'active']);
...
test('buildKnockoutStrategyEvolutionData appends failed experiments unless they beat the current flagship', () => {
  ...
  assert.equal(value.versions.at(-1).status, 'discarded');
  assert.match(value.versions.at(-1).verdict, /没有超过当前候选/);
