...
- `AI推荐` must feel like a special user: blue player chip, blue selected score chips, and per-match reason UI shown only when AI is selected. On mobile, show the reason preview directly under the match title before score options, allow two lines, and avoid internal strategy jargon. Historical ROI is a separate colored badge, not part of the reason text. Tapping the preview opens a scrollable dialog with larger text, the same ROI badge, one full reason body, and a large top-right return button; do not repeat summary plus detail, and do not use inline expand/collapse. Detail is capped at 400 characters.
