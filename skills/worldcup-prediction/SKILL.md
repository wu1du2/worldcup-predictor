...
- `AI推荐` must feel like a special user: blue player chip, blue selected score chips, and per-match reason UI shown only when AI is selected. On mobile, show the reason preview as a full-width explanation strip between the match header and score options, allow two lines, and avoid internal strategy jargon. Historical ROI is a separate colored badge, not part of the reason text. Tapping the preview opens a compact scrollable reading dialog with the same ROI badge and one full reason body; do not repeat summary plus detail, do not show extra dialog headings or match names, and do not use inline expand/collapse. Keep the top-right return control as a large icon button with an accessible label, and keep body text lighter than button/card labels so it reads like prose. Detail is capped at 400 characters.
