...
- Current production router candidates must match the three flagship strategies shown in the frontend knockout strategy view: stable `tem_draw_anchor_capped_1_draw5_5_cap35`, value `tem_poisson_drawguard_context_v3_n2_draw7_cap35_p0_006`, and consensus `tem_consensus_poisson_context_v1_c1_n4_cap7`. `low_score_basket_4` remains the no-odds fallback, not a normal routed candidate. Dynamic leaderboard candidates are offline/lab material unless explicitly re-enabled.
...
- Knockout value iteration lesson: simply adding more one-goal scores can improve hit count while destroying ROI via cost. A healthier pattern is conditional protection, e.g. `tem_poisson_drawguard_context_v3_n2_draw7_cap35_p0_006`: take two outcome-diverse Poisson EV picks, then add `1-1` only when its odds are no higher than 7. Preserve probability/EV fields for guard picks by reading from the model `evTable`, not only from selected top picks.
