...
- `AI推荐` must feel like a special user: blue player chip, blue selected score chips, and per-match reason UI shown only when AI is selected. On mobile, show the reason summary directly under the match title before score options, allow two lines, prefix it with historical ROI, and avoid internal strategy jargon. Tapping the summary opens a scrollable dialog with larger text and a large top-right return button; do not use inline expand/collapse. Detail is capped at 400 characters.
