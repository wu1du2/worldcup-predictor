...
- `AI推荐` must feel like a special user: blue player chip, blue selected score chips, and per-match reason UI shown only when AI is selected. On mobile, show the reason preview directly under the match title before score options, allow two lines, make the preview box use the full available title column, and avoid internal strategy jargon. Historical ROI is a separate colored badge, not part of the reason text. Tapping the preview opens a scrollable reading dialog with the same ROI badge and one full reason body; do not repeat summary plus detail, do not show extra dialog headings or match names, and do not use inline expand/collapse. The top-right return control should be a large icon button with an accessible label. Detail is capped at 400 characters.
